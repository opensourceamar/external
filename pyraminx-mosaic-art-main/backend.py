...
def sample_triangle_color(quantized_image, mask, method='mode', palette=None, original_image=None, precomputed_edges=None):
    ...
    base_img = np.asarray(original_image if original_image is not None else quantized_image)
    mask = np.asarray(mask)
    
    # 1. Get region coordinates from the mask
    coords = np.where(mask > 0)
    if len(coords[0]) == 0:
        return (255, 255, 255)

    # 2. Filter coordinates to ensure they are within the image boundaries
    h_i, w_i = base_img.shape[:2]
    valid_mask = (coords[0] < h_i) & (coords[1] < w_i)
    
    if precomputed_edges is not None:
        h_e, w_e = precomputed_edges.shape[:2]
        valid_mask &= (coords[0] < h_e) & (coords[1] < w_e)

    y_coords = coords[0][valid_mask]
    x_coords = coords[1][valid_mask]

    if len(y_coords) == 0:
        return (255, 255, 255)

    # 3. Extract pixel data safely using coordinate indexing
    pixels = base_img[y_coords, x_coords]
    ...
    if method == 'edge_aware' and original_image is not None and precomputed_edges is not None and palette is not None:
        try:
            ...
            # Sample edges using the same valid coordinates
            region_edges = precomputed_edges[y_coords, x_coords]
            
            # Separate edge pixels from center (non-edge) pixels
            is_edge = region_edges > 0
            edge_pixels = pixels[is_edge]
            center_pixels = pixels[~is_edge]

            # If not enough edge pixels, fall back to a simple mode of all pixels
            if len(edge_pixels) < 1:
                # Reshape pixels for Lab conversion (N, 3) -> (N, 1, 3)
            ...
            pass # Fall back to generic methods below
    ...
    # Generic methods (mean, center-ish)
    if method == 'mean':
        return quantize_color(np.mean(pixels, axis=0), palette)
    else:  # center-ish
        return quantize_color(pixels[len(pixels)//2], palette)
...
def create_triangle_mosaic(image, side_length, palette_size=4, use_dithering=False, sampling_method='mode', use_pyraminx_colors=True, palette_method='kmeans_lab', gamma_value=None, enhanceForDetail=False):
    ...
    # Re-sync new_height and new_width with the actual cropped image shape to avoid indexing mismatches
    new_height, new_width = original_input_image.shape[:2]
    ...
    # 2. Precompute edges for structural alignment (only once on the cropped image)
    precomputed_edges = detect_edges_with_bilateral(original_input_image) if sampling_method == 'edge_aware' else None
